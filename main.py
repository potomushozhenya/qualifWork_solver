...
class MonoImplicitSolver:
    ...
    def __init__(self, task = "simple"):
        ...
        self.y = np.array(np.array([]), dtype=np.float64)
    ...
    def res(self, y):
        ...
        k2 = np.append(k2, self.f2(x + self.c2[0] * self.h[-1], (1 - self.v21[0]) * y0_1 + self.v21[0] * y1 + self.x2[0, 0] * k1[0]))
    ...
    def solve(self, xFin, h0: float):
        ...
        while self.x[-1] < xFin:
            yRK = np.array(fsolve(self.res, self.y, xtol=10**-11), dtype=np.float64)
...
print(test.f1, test.f2, test.real_sol)
